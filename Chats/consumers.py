...
class ChatConsumer(AsyncWebsocketConsumer):
        ...
        return [
            {
                ...
                "picture": f"https://api.surytadz.com{msg.picture.url}" if msg.picture else None  # Check if picture exists
            }
            for msg in messages
        ]
    ...
    def get_receiver_info(self, conversation_id, current_user_id):
        ...
        try:
            ...
            return {
                ...
                "name": receiver.name,
                "profile_picture": f"https://api.surytadz.com{receiver.profile_picture.url}" if receiver.profile_picture else None
            }
        except Conversation.DoesNotExist:
    ...
    async def receive(self, text_data):
        ...
        sender_data = await database_sync_to_async(lambda: {
            ...
            "profile_picture": f"https://api.surytadz.com{sender_marketuser.profile_picture.url}" if sender_marketuser.profile_picture else None
        })()

        recipient_data = await database_sync_to_async(lambda: {
            "id": recipient_marketuser.profile.pk,
            "username": recipient_marketuser.name,
            "profile_picture": f"https://api.surytadz.com{recipient_marketuser.profile_picture.url}" if recipient_marketuser.profile_picture else None
        })()
        ...
        if message:
            ...
            await self.channel_layer.group_send(
                self.room_group_name,
                {
                    ...
                    "picture": f"https://api.surytadz.com{message.picture.url}" if message.picture else None,  # ✅ Correct URL
